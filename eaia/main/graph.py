...
def send_cal_invite_node(state, config):
    ...
    return {
        "calendar_invite_sent": sent,
        "messages": [ToolMessage(content=message, tool_call_id=tool_call["id"])],
    }


def route_after_calendar_send(state: State) -> Literal["mark_as_read_node", "draft_response"]:
    if state.get("calendar_invite_sent"):
        return "mark_as_read_node"
    return "draft_response"
...
graph_builder.add_conditional_edges("send_cal_invite_node", route_after_calendar_send)
